...
export default function TrackPage() {
  ...
  if (!order) return (
    <div className="min-h-screen flex items-center justify-center text-sm text-gray-400">Order not found.</div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-black text-white px-6 py-8 text-center">
        ...
        <p className="text-gray-400 text-sm mt-1">{order.order_number} · {order.shops?.name}</p>
      </div>
      ...
      <div className="max-w-lg mx-auto px-6 py-8 space-y-6">
              ...
              return (
                <div key={step.status} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className={`text-sm font-medium ${active ? "text-gray-900" : done ? "text-gray-500" : "text-gray-300"}`}>
                      {step.label}
                    </p>
                    {active && (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 space-y-3">
          ...
          <div className="border-t border-gray-100 pt-3 space-y-1 text-xs text-gray-500">
            ...
            <div className="flex justify-between"><span>Delivery</span><span>${Number(order.delivery_fee).toFixed(2)}</span></div>
        ...
        {order.shops?.phone && (
          <div className="bg-white border border-gray-200 rounded-2xl p-5 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm font-medium text-gray-900">Contact {order.shops.name}</p>
            </div>
            <a
